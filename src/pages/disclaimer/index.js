...
function DesktopDisclaimer({ isChinese }) {
  return (
...
export default function Disclaimer() {
  ...
  const manuallySelectedLanguage = canUseDOM ? window.localStorage.getItem('manuallySelectLanguage') : null;
  const isChinese = manuallySelectedLanguage === 'zh-Hans';
  const showMobile = canUseDOM ? Boolean(isMobile()) : false;

  return (
    <Layout>
      {showMobile ? <MobileDisclaimer /> : <DesktopDisclaimer isChinese={isChinese} />}
    </Layout>
  );
}
